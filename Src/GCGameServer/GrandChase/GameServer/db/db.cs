...
namespace GameServer.db
{
    class DBConnect
    {
        ...
        private string server = configserver.server;
        private string database = configserver.database;
        private string uid = configserver.uid;
        private string password = configserver.password;
        ...
        public void Exec(DataSet ds,string query)
        {
            using (MySqlConnection connection = new MySqlConnection(string.Format("SERVER={0}; UID={1}; PASSWORD={2}; DATABASE={3};", configserver.server, configserver.uid, configserver.password, configserver.database)))
            {
                ...
                adapter.SelectCommand = new MySqlCommand(string.Format(query), connection);
                adapter.Fill(ds);
                connection.Close();
            }
        }
        ...
        public void Backup()
        {
            ...
            {
                log.Write("Error , unable to backup!", ex);
            }
        }
        ...
        public void Restore()
        {
            ...
            {
                log.Write("Error , unable to Restore!", ex);
            }
        }
    }
}
